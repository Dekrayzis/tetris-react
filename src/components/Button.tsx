...
const StyledButton = styled.button<StyledButtonProps>`
  ...
  margin: ${props => props.$startBtn ? '0' : '0 0 6px 0'};
  ...
  outline: none;
  cursor: pointer;
  position: ${props => props.$customStyle?.position ? `${props.$customStyle.position}` : 'absolute'};
  left: ${props => props.$customStyle?.left ? `${props.$customStyle.left}` : 'auto' };
  top: ${props => props.$customStyle?.top ? `${props.$customStyle.top}` : 'auto' };
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ callback, isStartBtn, label, customStyle = {}, autoFocus, type = 'button' }, ref) => (
    <StyledButton
      ref={ref}
      type={type}
      autoFocus={autoFocus}
      className={isStartBtn ? 'pulsate' : 'float-shadow'}
      $startBtn={isStartBtn}
      onClick={callback}
